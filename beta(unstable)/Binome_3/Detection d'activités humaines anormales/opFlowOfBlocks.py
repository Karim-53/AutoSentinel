...
def calcOptFlowOfBlocks(mag,angle,grayImg):
    ...
    opFlowOfBlocks = np.zeros((math.floor(xBlockSize),math.floor(yBlockSize),2))
    
    for index,value in np.ndenumerate(mag):
        opFlowOfBlocks[math.floor(index[0]/noOfRowInBlock)][math.floor(index[1]/noOfColInBlock)][0] += mag[math.floor(index[0])][math.floor(index[1])]
        opFlowOfBlocks[math.floor(index[0]/noOfRowInBlock)][math.floor(index[1]/noOfColInBlock)][1] += angle[math.floor(index[0])][math.floor(index[1])]

    centreOfBlocks = np.zeros((math.floor(xBlockSize),math.floor(yBlockSize),2))
