    ...
    for j in range(num):
        for i in range(meta.classes):
            if dets[j].prob[i] > 0:
                ...
                #cv2.rectangle(im,(b.x, b.y), (b.w, b.h),(0,255,0),2)
    res = sorted(res, key=lambda x: -x[1])
    free_image(im)
    free_detections(dets, num)
    return res, im

def detect1(net, meta, im, thresh=.5, hier_thresh=.5, nms=.45):
    num = c_int(0)
    pnum = pointer(num)
    predict_image(net, im)
    dets = get_network_boxes(net, im.w, im.h, thresh, hier_thresh, None, 0, pnum)
    ...
    res = sorted(res, key=lambda x: -x[1])
    free_image(im)
    free_ptrs(cast(dets[i].prob[i], POINTER(c_void_p)), num)
    return res
...
if __name__ == "__main__":
    ...
    r,im = detect(net, meta,"dog.jpg".encode("utf-8"))
    #detectAndShow(load_image("dog.jpg".encode("utf-8"), 0, 0))
    #cv2.imshow(im)
